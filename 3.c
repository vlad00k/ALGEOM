...
int get_row_idx(float *matrix[], int d, int dementions);
...
int is_new_basis(int vector[], float *basis[], int dementions)
{
	if (is_zero_basis_vector(basis[0], dementions)) // no basis vectors
		return 1;
...
int is_linear_combination(float *matrix[], float *basis[], int vector[], int dementions, int column_number)
{
	...
	{
		// printf("Demention %d\n", d);
		// print_matrix(matrix, dementions, column_number);
		// printf("\n");
		row_idx = get_row_idx(matrix, d, dementions);
		if (row_idx == -1)
		{
